#!/usr/bin/env node
/**
 * Propose removing an authorized issuer from the resolver.
 *
 * Usage:
 *   npm run admin:propose-resolver-remove-issuer -- --network omachainTestnet --issuer 0x...
 */
...
function printUsage(): void {
  console.log(`
Usage: propose-resolver-remove-issuer --network <network> --issuer <address>

Proposes removing an authorized issuer from the resolver via the timelock.

Options:
  --network       Target network (required)
  --issuer        Address to remove as issuer (required)
...
async function main(): Promise<void> {
  ...
  const isAuthorized = await resolver.getFunction('isIssuer')(issuer);
  if (!isAuthorized) {
    console.log('\n⚠️  Address is not an authorized issuer. Nothing to do.');
    return;
  }

  const iface = new ethers.Interface(RESOLVER_ABI);
  const calldata = iface.encodeFunctionData('removeAuthorizedIssuer', [issuer]);
  ...
  if (config.timelock) {
    await submitTimelockProposal({
      ...
      calldata,
      description: `remove-issuer-${issuer}`,
    });
  } else if (directCall) {
